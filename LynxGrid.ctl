...
'// Modified: 09/08/08
'// Version:  2.14.2 (see HistoryLog file for details)
...
Private mudtTotals()    As typTotals   '// MLH
Private mudtTotalsVal() As Double      '// MLH
...
Private miKeyCode            As Integer '// MLH - Used in Drawgrid (when workwrap is on) and UserControl_KeyDown for Edit
'   mode.
...
'''                             Optional ByVal lColWidth As Long = 2000)
'''
...
'''             Me.AddColumn fd.Name, lColWidth
'''         Next
'''      End If
'''
...
Public Function AddColumn(Optional ByVal Caption As String, _
  ...
  Dim lNewCol As Long
   ...
   If Not (mCols(0).nAlignment = 0) Then
      ...
      ReDim Preserve mudtTotals(lNewCol) As typTotals    '// MLH
      ReDim Preserve mudtTotalsVal(lNewCol) As Double    '// MLH
...
Public Function AddItem(Optional ByVal vstrItem As String, _
                        ...
                        Optional ByVal vbRowVisible As Boolean = True) As Long
   ...
   '// MLH - Changed Optional vRow As Long = 0, Added vbMarkAsNew
   '---------------------------------------------------------------------------------
  ...
  Dim sText() As String
   ...
   If UBound(mCols) > 0 Then
      ...
      For lCount = LBound(sText) To UBound(sText)
         ...
         ElseIf mCols(lCol).nType = lgNumeric Then '// MLH - New
            mudtTotalsVal(lCol) = mudtTotalsVal(lCol) + rVal(sText(lCount))
         End If
   ...
   Else
      ...
      ElseIf mCols(lCol).nType = lgNumeric Then '// MLH - New
         mudtTotalsVal(lCol) = mudtTotalsVal(lCol) + rVal(vstrItem)
      End If
...
Private Sub ApplyCellFormat(ByVal vRow As Long, _
  ...
  Dim bMatch      As Boolean
   ...
   For lCount = 0 To UBound(mCF)
      ...
      'If (mCF(lCount).lBackColor = lBackColor) And (mCF(lCount).lForeColor = lForeColor) And (mCF(lCount).nImage = nImage)
      '   And (mCF(lCount).sFontName = sFontName) Then '// MLH

      If (mCF(lCount).lBackColor = lBackColor) And (mCF(lCount).lForeColor = lForeColor) And (mCF(lCount).sFontName = _
         sFontName) Then
...
Public Property Let CellText(Optional ByVal vRow As Long = C_NULL_RESULT, _
   ...
   If FixRef(vRow, vCol) Then '// MLH
      ...
      ElseIf mCols(vCol).nType = lgNumeric Then
...
Public Property Let ColAlignment(ByVal Index As Long, ByVal vNewValue As lgAlignmentEnum)

   mCols(Index).nAlignment = vNewValue
   DrawGrid mbRedraw
...
Public Property Let ColFormat(ByVal Index As Long, ByVal vNewValue As String)

   mCols(Index).sFormat = vNewValue
   DrawGrid mbRedraw
...
Public Property Let ColHeading(ByVal Index As Long, ByVal vNewValue As String)

   mCols(Index).sCaption = vNewValue
   DrawGrid mbRedraw
...
Public Property Let ColImageAlignment(ByVal Index As Long, ByVal vNewValue As lgAlignmentEnum)

   mCols(Index).nImageAlignment = vNewValue
   DrawGrid mbRedraw
...
Public Property Let ColInputFilter(ByVal Index As Long, ByVal vNewValue As String)

   mCols(Index).sInputFilter = vNewValue

End Property

Public Property Get ColLocked(ByVal Index As Long) As Boolean

   If Index = C_NULL_RESULT Or mLCLocked Then
      ColLocked = True
   Else
      ColLocked = mCols(Index).bLocked
   End If

End Property

Public Property Let ColLocked(ByVal Index As Long, ByVal vNewValue As Boolean)

   mCols(Index).bLocked = vNewValue
   DrawGrid mbRedraw
...
Public Sub ColForceFit()
  ...
  Dim dblWidth    As Double
   ...
   For lngI = 0 To UBound(mCols)
      ColWidth(lngI) = VisibleWidth * dblWidth
   Next lngI
   
   mbPendingRedraw = True
   SetRedrawState True
   If mbUserRedraw Then Refresh
...
Public Property Let ColTag(ByVal Index As Long, ByVal vNewValue As String)

   mCols(Index).sTag = vNewValue
...
Public Property Let ColType(ByVal Index As Long, ByVal vNewValue As lgDataTypeEnum)

   mCols(Index).nType = vNewValue
...
Public Property Let ColVisible(ByVal Index As Long, ByVal vNewValue As Boolean)

   mCols(Index).bVisible = vNewValue

   DrawGrid mbRedraw

End Property

Public Sub ColWidthAutoSize(ByVal Index As Long)
   ...
   Case Else
      With UserControl
         ...
         If mbTotalsLineShow Then
            If mCols(Index).nType = lgNumeric Then
               strTemp = mudtTotals(Index).sCaption & Format$(mudtTotalsVal(Index), mCols(Index).sFormat)
               lngL = .TextWidth(strTemp)
            End If
         End If
            
         For lngR = 0 To mRowCount
            .FontBold = mItems(lngR).Cell(Index).nFlags And lgFLFontBold
            .FontItalic = mItems(lngR).Cell(Index).nFlags And lgFLFontItalic
            .FontUnderline = mItems(lngR).Cell(Index).nFlags And lgFLFontUnderline
            .FontName = mCF(mItems(lngR).Cell(Index).nFormat).sFontName
            
            strTemp = Format$(mItems(lngR).Cell(Index).sValue, mCols(Index).sFormat)
            ...
            If Index = 0 Then
               ...
               End If
            ElseIf mItems(lngR).Cell(Index).lImage Then
               lngW = lngW + mR.ImageWidth
            End If
      ...
      ColWidth(Index) = (lngL + Screen.TwipsPerPixelX) * Screen.TwipsPerPixelX
   End Select
...
Public Property Let ColWidth(ByVal Index As Long, ByVal vNewValue As Single)

   '// dCustomWidth is in the Units the Control is operating in
   On Error Resume Next
   mCols(Index).dCustomWidth = vNewValue
   mCols(Index).lWidth = ScaleX(vNewValue, muScaleUnits, vbPixels)
...
Public Property Let ColWordWrap(ByVal Index As Long, ByVal vNewValue As Boolean)

   SetFlag mCols(Index).nFlags, lgFLWordWrap, vNewValue
...
Private Sub DrawGrid(ByVal bRedraw As Boolean)
  ...
  Dim bToggle        As Boolean
   ...
   If bRedraw Then
      ...
      With UserControl
         ...
         If mRowCount < 0 Then Exit Sub  '// MLH - exit sub if there is nothing to do
   ...
   '                           lValue = ((mCols(mColPtr(lCol)).lWidth - 2) / 100) *
   '                           mItems(mRowPtr(lRow)).Cell(mColPtr(lCol)).nFlags
   '                           SetRect R, lX + 2, lY + 2, lX + lValue, (lY +
   '                           mItems(mRowPtr(lRow)).lHeight) - 2
   ...
   '                        End If
                        ...
                        Case Else
                           ...
                           With mItems(mRowPtr(lRow)).Cell(mColPtr(lCol))
                              ...
                              If mCols(mColPtr(lCol)).nType = lgProgressBar Then '// MLH - Progress bar value is stored in .nFlags
         ...
         If Not (muFocusRectMode = lgFocusRectModeEnum.lgNone) And (mRow >= 0) Then
            If Not mbHideSelection Or mbInFocus Then
            ...
            End If '// Not mbHideSelection Or mbInFocus
         End If
...
Public Sub FormatCells(ByVal RowFrom As Long, _
                       ByVal RowTo As Long, _
                       ByVal ColFrom As Long, _
                       ByVal ColTo As Long, _
                       ByVal Mode As lgCellFormatEnum, _
                       ByVal vNewValue As Variant)
  ...
  Dim sValue As String
   ...
   If Not (mRowCount = C_NULL_RESULT) Then '// MLH - prevent error
      ...
      Case lgCFBackColor, lgCFForeColor, lgCFImage
         lValue = CLng(vNewValue)

      Case lgCFFontName
         sValue = CStr(vNewValue)

      Case lgCFFontBold, lgCFFontItalic, lgCFFontUnderline
         bValue = CBool(vNewValue)
      End Select
   ...
   Else
      ...
      If (TypeOf mCols(mColPtr(mEditCol)).EditCtrl Is VB.ComboBox) Then
         ...
         With mCols(mColPtr(mEditCol)).EditCtrl
            ...
            If mCols(mColPtr(mEditCol)).MoveControl And lgBCHeight Then
               lHeight = mMinRowHeight - mGridLineWidth - 4
      ...
      Else
         ...
         With mCols(mColPtr(mEditCol)).EditCtrl
            ...
            If mCols(mColPtr(mEditCol)).MoveControl And lgBCTop Then
               .Top = ScaleY(RowTop(mEditRow) + mGridLineWidth + lBorderWidth, vbPixels, nScaleMode) + _
  ...
  Dim bSelected As Boolean
   ...
   If mRowCount >= 0 Then
      ...
      For lCount = 0 To UBound(mCols)
         ...
         If mCols(lCount).nType = lgNumeric Then
...
Public Function RowAdded(ByVal vRow As Long) As Boolean
   ...
   '// MLH - new sub
   RowAdded = mItems(vRow).nFlags And lgFLNewRow
  ...
  Dim lCol As Long
   ...
   If FixRef(vRow) Then
      For lCol = LBound(mCols) To UBound(mCols)
         CellBackColor(vRow, lCol) = vNewValue
      Next lCol
...
Public Function RowChanged(ByVal vRow As Long) As Boolean
   ...
   '// MLH - new sub
   RowChanged = mItems(vRow).nFlags And lgFLChanged
...
Public Sub RowColSet(Optional ByVal lNewRow As Long = C_NULL_RESULT, Optional ByVal lNewCol As Long = C_NULL_RESULT)

   '// MLH - New
  Dim bRedraw As Boolean
  Dim lngI    As Long

   If FixRef(lNewRow, lNewCol) Then
      If mItems(mRowPtr(lNewRow)).bVisible And mCols(mColPtr(lNewCol)).bVisible Then
      
         '// FIND MOVED COLUMN NUMBER
         lNewCol = mColPtr(lNewCol)
...
Public Property Let RowFontBold(Optional ByVal vRow As Long = C_NULL_RESULT, ByVal vNewValue As Boolean)
  ...
  Dim lCol As Long
   ...
   If FixRef(vRow) Then
      For lCol = LBound(mCols) To UBound(mCols)
         CellFontBold(vRow, lCol) = vNewValue
      Next lCol
...
Public Property Let RowForeColor(Optional ByVal vRow As Long = C_NULL_RESULT, ByVal vNewValue As Long)
  ...
  Dim lCol As Long
   ...
   If FixRef(vRow) Then
      For lCol = LBound(mCols) To UBound(mCols)
         CellForeColor(vRow, lCol) = vNewValue
      Next lCol
...
'               End Select
      ...
      For lCount = LBound(mRowPtr) To mRowCount
         If mRowPtr(lCount) = lRowIndex Then
            'mRow = lCount
            RowColSet lCount '// MLH - keep selected row visible
            Exit For
         End If
      Next lCount
   ...
   If mbEditPending Then
      ...
      If Not bCancel Then
         ...
         If bRequestUpdate Then
            ...
            If mCols(mColPtr(mEditCol)).nType = lgNumeric Then '// MLH - new
...
Private Sub UserControl_Initialize()
  ...
  Dim OS As OSVersionInfo
   ...
   mbLockFocusDraw = IsWindowUnicode(UserControl.hWnd)
            ...
            If muFocusRectMode = lgCol Then '// FocusRect = Col
               ...
               If Not (mCol = C_NULL_RESULT) Then
                  If Not (mCols(mColPtr(mCol)).nType = lgBoolean Or mCols(mColPtr(mCol)).bLocked) Then
                     ...
                     If Not bCancel Then
                        If mCols(mCol).nType = lgNumeric Then
